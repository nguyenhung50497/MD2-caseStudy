...
function addAccount() {
    ...
    let str = userManager.showAllUsers().filter(name => name.userName.toUpperCase() === userName.toLowerCase())
    let password = input.question('Mat khau: ', {hideEchoBack: true});
    let newUser = new User(id, userName.toUppercase(), password);
...
function editAccount() {
    ...
    do {
        ...
        if (choice === 0) {
            ...
            let newAccount = new User(account.id, userName, password);
