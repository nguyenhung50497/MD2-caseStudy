...
function displayComp(arr: Computer[]) {
    ...
    for (let i = 0; i < arr.length; i++) {
        str += `${i+1}: ${arr[i].name} - Trạng thái: ${arr[i].status} - Giờ chơi: ${arr[i].hours}h - Tổng tiền: ${buyService()}$\n`;
        ...
        if (choice === 0) {
            ...
            for (let i = 0; i < serviceManager.showAllServices().length; i++) {
                ...
                if (choice2 === 0) {
                    ...
                    return cyberGame.showAllComputers()[choice-1].totalMoney += (newService.price + cyberGame.showAllComputers()[choice-1].hours*2);
                }
            } while (choice !== 0)
        }
    } while (choice !== 0)
