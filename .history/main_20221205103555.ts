...
let may1 = new Computer(1, 'offline', new Date(), 0);
let may2 = new Computer(2, 'online', new Date(), 6);
let may3 = new Computer(3, 'offline', new Date(), 0);
let may4 = new Computer(4, 'offline', new Date(), 0);
let may5 = new Computer(5, 'online', new Date(), 10);
...
function addComputer() {
    let id = cyberGame.showAllComputers().length+1;
    let newComputer = new Computer(id, 'offline', new Date(), 0);
...
function editComputer() {
    ...
    do {
        ...
        if (choice === 0) {
            ...
            ${choice}: Máy ${computer.getId()} - Tình trạng: ${computer.getStatus()}`);
...
function payBill() {
    ...
    do {
        ...
        if (choice === 0) {
            ...
            let time = new Date;
            let dateMoney = new DateMoney(time.toLocaleDateString(), sum);
            if (dateMoneyManager.showAll().length === 0) {
                dateMoneyManager.addDateMoney(dateMoney);
            }
            else {
                for (let i of dateMoneyManager.showAll()) {
                    if (i.day !== dateMoney.day) {
                        dateMoneyManager.addDateMoney(dateMoney);
                    }
                    else {
                        i.money += dateMoney.money;
                    }
                }
            }
            cyberGame.showAllComputers()[choice-1].setStatus('offline');
            cyberGame.showAllComputers()[choice-1].setTime(new Date());
            cyberGame.showAllComputers()[choice-1].setMoneyService(0);
        ...
        if (choice === 0) {
            ...
            console.log(`0. Thoát`);
                ...
                if (choice2 === 0) {
                    ...
                    cyberGame.showAllComputers()[choice-1].setMoneyService(cyberGame.showAllComputers()[choice-1].getMoneyService) += newService.price * quantity;
                    buyService();
                    break;
                }
            } while (choice !== 0)
...
function dayMoneyFromTo() {
    console.log(`------------------------Doanh thu theo ngày---------------------`);
    let start = input.question('Enter start day: ');
    let end = input.question('Enter end day: ');
    let arr = dateMoneyManager.showAll();
    let sum = 0;
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].day === start) {
            arr.splice(0, i);
    ...
    console.log(`Doanh thu từ ngày ${start} đến ngày ${end}: ${sum}$`);
    
}

function dayMoney() {
    console.log(`------------------------Doanh thu theo từng ngày---------------------`);
    for (let i = 0; i < dateMoneyManager.showAll().length; i++) {
        console.log(`${i+1}: ${dateMoneyManager.showAll()[i].day} - Doanh thu: ${dateMoneyManager.showAll()[i].money}$`);
    }
}